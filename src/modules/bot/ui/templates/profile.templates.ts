...
export const PROFILE_MESSAGES = {
  ...
  PROFILE: (user: User, rental: RentalData | undefined) => {
    const subscriptionEmoji =
      user.subscriptionType === 'premium' ? EMOJI.PREMIUM : EMOJI.FREE;
    const statusEmoji =
      user.status === 'active' ? EMOJI.ACTIVE : EMOJI.INACTIVE;
  ...
  NO_ACTIVE_SERVERS:
    `${EMOJI.NO} *У вас нет активных серверов*\n\n` +
    `Арендуйте узел в главном меню, чтобы получить доступ.`,

  REFERRAL_TITLE:
    `${EMOJI.REFERRAL} *Партнерская программа*\n\n` +
